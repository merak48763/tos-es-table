...
function generate_quiz_table(arg) {
    ...
    tokens[1].forEach(element => {
        ...
        const question_image = qa.quizImage ? `<br /><img class="question_image" src="/tool_data/image/quiz_image/${qa.quizImage}" />` : "";
        table_rows_html += `<tr><td>${qa.question}${question_image}</td><td>${answer_string}</td><td>${answer_monster}</td></tr>`;
    });
...
const annotation_replacers = {
    ...
    'explode_increase_attack': () => `<ul><li>移除增攻：每1顆增加0.5倍，最高20顆增加10倍</li></ul>`,
    'explode_reduce_damage': arg => `<ul><li>移除減傷：每1顆減傷${arg || '5'}%，最高${Math.ceil(100/parseInt(arg || '5'))}顆減傷100%</li></ul>`,
    'mild_attack': () => `<ul><li>輕量攻擊：敵人攻擊力的65%</ul></li>`,
